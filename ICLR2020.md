...
- 负责 | 沈硕
- 摘要：model-based与MPC结合的方法在运动控制领域有不错的样本效率和渐进表现以及很大的潜力。现有的规划方法从动作空间中的随机序列中选择最好的，通常在高维空间中难以适用。我们提出了POPLIN，一种结合policy网络与online planning的方法。并在实验中把基于policy网络初始化的动作序列与随机动作序列对比。同时在mujoco中和PETS、SAC、TD3对比展示出了三倍以上的样本效率。通过优化空间上表面的光滑程度解释了我们的算法。
- 创新模式：老问题首次引入某种新技术或提出新方法或改进
- 技术总结：基于模型，单agent，不确定性，老问题首次引入某种新技术或提出新方法或改进
- 关键词：策略网络，CEM，shooting methond，集成学习
- 源代码：https://github.com/WilsonWangTHU/POPLIN
- 贡献：我们提出了结合policy网络和online planning的方法，在mujoco上达到了sota的表现。研究了不同策略提取方法的效果。
...
- 负责 | 宋明惠
- 摘要 | 在稀疏奖励环境下的探索仍然是无模型强化学习的关键挑战之一。许多先进的方法不仅仅依靠外部环境提供的奖励，而是使用内部奖励来鼓励探索。然而本文表明，在程序生成环境中，一个智能体不可能多次访问某个状态，因此现有的方法存在一些弊端。本文提出了一种新型的内在奖励，它鼓励智能体采取导致其学习状态表征发生重大变化的行动。本文在MiniGrid中对多个具有挑战性的程序生成任务，以及在以前的工作中使用的具有高维观察的任务中评估本文的方法。实验表明，这种方法比现有的探索方法具有更高的样本效率，特别是对于程序生成的微型环境。此外，我们分析了学习到的行为以及智能体所获得的内在奖励。与之前的方法相比，本文的内在奖励不会在训练过程中减少，而且它会因为智能体与它所能控制的对象进行互动而给予智能体更多的奖励。
- 创新模式 | 老问题首次引入某种新技术或提出新方法或改进
- 技术总结 | 无模型的，单智能体
- 关键词 | 部分可观测，变分循环模型(VRM)，Model-free
- 源代码 | https://github.com/facebookresearch/impact-driven-exploration
- 贡献 | 本文提出奖励影响驱动的探索（RIDE），这是一种内在奖励，鼓励智能体探索大致上可以改变环境状态的行为。
...
- 技术总结 | 无模型，单智能体
- 关键词 | 分布式，DD-PPO
- 源代码 | <https://github.com/facebookresearch/habitat-api >
...
- 链接 | https://arxiv.org/pdf/2004.06089.pdf
...
- 负责 | 宋明惠
- 摘要 | 我们研究在以下环境中进行强化学习的情况：必须在受控系统的时间演变过程中同时从策略中采样一个动作，例如当机器人必须在决定下一个动作的同时仍执行前一个动作。就像人或动物一样，机器人必须同时思考和移动，在上一个动作完成之前决定下一个动作。为了开发针对此类并发控制问题的算法框架，我们从Bellman方程的连续时间公式化开始，然后以意识到系统延迟的方式离散化它们。通过对现有基于值的深度强化学习算法的简单体系结构扩展，我们实例化了此类新的近似动态编程方法。我们在模拟基准任务和大型机器人抓取任务上评估我们的方法，而机器人在移动时必须思考。可以在https://sites.google.com/view/thinkingwhilemoving上观看视频
- 创新模式 | 老问题首次引入某种新技术或提出新方法或改进
- 技术总结 | on-value
- 关键词 | 并发强化学习
- 源代码 | 暂无
- 贡献 | 提出了一个分析并行系统的理论框架，其中智能体再移动时必须思考。引入了Vector-to-go (VTG)，它结合了要执行的其余先前动作，作为先前动作和t时刻的动作状态所包含的并发系统信息的替代表示。该理论发现得到了q学习模型在模拟控制任务中的实验结果的支持。
...
- 负责 | 宋明惠
- 摘要 | 视频预测模型与规划算法相结合，使机器人能够通过自我监督学习执行许多基于视觉的任务，在有看不见的物体的杂乱场景中达到新的目标。然而，由于长视界视频预测的复合不确定性和基于采样的规划优化器的可伸缩性较差，这些方法的一个重要限制是能够进行长视界规划以达到遥远的目标。为此，我们提出了一种子目标生成和规划的框架——分层视觉预见(HVF)，它根据目标图像生成子目标图像，并将其用于规划。子目标图像是直接优化的任务分解成易于规划的片段，因此，我们观察到该方法自然地识别语义有意义的状态为子目标。在四个模拟的基于视觉的操作任务中，我们发现我们的方法比没有子目标和无模型RL方法的规划获得了超过20%的绝对性能改进。此外，我们的实验表明，我们的方法扩展到真实的、杂乱的视觉场景。
- 创新模式 | 老问题首次引入某种新技术或提出新方法或改进
- 技术总结 | 分层视觉预见(HVF)
- 关键词 | 单智能体 视觉 有模型
- 源代码 | 暂无
- 贡献 | 提出了一种基于视觉任务的分层规划的自我监督方法，即分层视觉预见(HVF)，它将一个视觉目标分解为一系列子目标。通过明确优化子目标，使规划成本最小化，HVF能够在视觉空间中发现语义上有意义的目标，并在使用这些目标进行规划时，执行各种具有挑战性的、基于长期视野的任务。
...
- 负责 | 王翔森
- 摘要 | 强化学习（RL）经常用于提高文本生成任务（包括机器翻译（MT））的性能，特别是通过使用最小风险培训（MRT）和生成性对抗网络（GAN）。 然而，关于这些方法在MT的背景下学习的内容和方式知之甚少。 我们证明MT最常见的RL方法之一并没有优化预期的奖励，并且表明其他方法需要不可思议的长时间来收敛。 实际上，我们的结果表明，只有在预先训练好的参数已接近产生正确平移的情况下，MT中的RL实践才有可能提高性能。 我们的研究结果进一步表明观察到的增益可能是由于与训练信号无关的影响，而是由于分布曲线形状的变化。
- 创新模式 | 理论文章，论述强化学习在机器翻译上的缺点
- 技术总结 | 机器翻译 最小风险培训（MRT）和生成性对抗网络（GAN）
- 关键词 | 机器翻译 强化学习 弱点
- 源代码 | 暂无
- 贡献 | 我们证明MT最常见的RL方法之一并没有优化预期的奖励，并且表明其他方法需要不可思议的长时间来收敛。 实际上，我们的结果表明，只有在预先训练好的参数已接近产生正确平移的情况下，MT中的RL实践才有可能提高性能。
...
- 负责 | 宋明惠
- 摘要 | 随着视觉感知驱动的深度强化学习的广泛应用，对已学习agent的理解和探索的需求越来越大。理解决策过程及其与视觉输入的关系对于识别习得行为中的问题非常有价值。然而，这一课题在研究界的探索相对较少。在这项工作中，我们提出了一种方法来综合视觉输入感兴趣的训练agent。此类输入或状态可能是需要采取特定措施的情况。此外，能够获得非常高或非常低回报的关键状态对于理解系统的情境感知通常是很有趣的，因为它们与风险状态相对应。为此，我们学习了环境状态空间的生成模型，并利用它的潜在空间来优化目标函数的利益状态。在我们的实验中，我们表明这种方法可以产生对各种环境和强化学习方法的见解。我们将在标准的Atari基准游戏以及自动驾驶模拟器中探索结果。基于我们利用该技术识别行为弱点的效率，我们相信这一通用方法可以作为人工智能安全应用的重要工具。
- 创新模式 | 老问题首次引入某种新技术或提出新方法或改进
- 技术总结 | 单agent 特征模型可视化
- 关键词 | 深度强化学习、可视化
- 源代码 | 无
- 贡献 | 提出了一种基于环境和用户定义的目标函数的方法来综合深度强化学习智能体的输入。agent的感知损失有助于重建集中于当前状态对agent很重要的区域，避免产生欺骗的例子。训练生成器生成代理感知到的状态，就像从真实环境中感知到的状态一样，这可以优化其潜在空间，以采样感兴趣的状态。
...
- 单位 ｜ UC Berkeley
- 负责 | 沈硕
- 摘要 | 大部分model-based 方法把model当作黑盒生成额外的数据供策略或值函数学习，这篇文章中我们利用model的可微性来展示如何更高效地利用model。我们构建了一种基于轨迹的梯度学习策略的方法，通过增加了值函数解决长轨迹学习不稳定的问题，适用actor critic的架构学习。最后根据梯度和值函数的误差提出了一个渐进性提升的证明。
- 创新模式 ｜ 老问题首次引入某种新技术或提出新方法或改进
- 技术总结 ｜ 基于模型的， 单agent
- 关键词 ｜ model-based，backprop though path，actor critic，sample efficient
- 源代码 ｜无
- 贡献 ｜1、我们的模型在样本效率上达到了sota的表现
        2、在最终表现上与model free 方法持平
        3、可以应用在在通常model-based方法表现不好的长视野问题上
...
- 负责 | 赵亮
- 摘要 | Q-Learning存在高估偏差的问题，因为Q-Learning使用最大估计动作值来近似最大动作值。已经有很多算法被提出来解决高估偏差的问题，但是我们对偏差与表现是如何相互作用的，以及对现有算法减轻偏差的效果缺乏了解。在本文中：1）我们强调高估偏差对学习效率的影响取决于环境；2）我们提出了一种广义的Q-learning方法，称为Maxmin Q-learning，它提供了一个参数来灵活地控制偏差；3）我们从理论上证明，Maxmin Q-learning存在一个参数选择，该参数选择导致无偏估计，且逼近方差比Q-learning低。4）我们使用新的广义Q-learning框架证明了我们算法的收敛性，以及以前的几种Q-learning变体的收敛性。我们凭经验验证了我们的算法可以更好地控制玩具环境中的估计偏差，并且可以在几个基准问题上实现出色的性能。
- 创新模式 | 老问题首次引入某种新技术或提出新方法或改进（一种新的解决Q-Learning过高估计的办法）
- 技术总结 | 无模型，单智能体
- 关键词 | Q-learning
- 源代码 | https://github.com/qlan3/Explorer.git
- 贡献 | 提出了一种广义的Q-learning方法，称为Maxmin Q-learning，它提供一个参数来灵活地控制偏差.
...
- 负责 | 胡小波
- 摘要 | 设计生物结构（例如DNA或蛋白质）的能力将对医学和工业产生重大影响。这样做提出了一个具有挑战性的黑箱优化问题，因为其需要劳动密集型的湿实验室评估，所以其具有着大批量，低周期设置的特点。针对这一问题，我们提出了基于最近策略优化（PPO）的强化学习（RL）方法来进行生物序列设计。 RL为优化生成序列模型提供了灵活的框架，以实现特定的标准，例如发现的高质量序列之间的多样性。为了提高样本效率，我们提出了一种基于模型的PPO变体DyNA-PPO，其中新一轮的策略是使用前几轮的模拟拟合功能测量来避免新一轮的脱机。为了适应各轮之间不断增长的观测数据，模拟器模型在每轮都会从不同容量的不同模型池中自动选择。在DNA转录因子结合位点的设计、抗菌蛋白的设计以及基于蛋白质结构优化Ising模型能量的任务上，我们发现DyNA-PPO在建模可行的情况下比现有的方法有显著的提高，但在无法学习可靠模型的情况下仍然没有表现得更差。
- 创新模式 | 将某个问题首次使用强化学习进行建模
- 技术总结 | 有模型，单智能体， 确定性
- 关键词 | 模拟器， 序列生成， 模拟仿真
- 源代码 | 暂未发布
- 贡献 | 综上所述，我们的贡献如下：
1我们提供了一个基于模型的RL算法DyNA-PPO，并证明了其在生物序列设计所施加的大批量/低轮约束下的有效性。
2我们证明基于模型的RL可以通过量化可靠性和通过交叉验证自动选择适当复杂度的模型来提高样本效率和消除模型偏差。
3我们提出了一种基于访问的探索红利，并表明它比熵正则化更有效地识别多个局部最优。
4我们提出了一个基于蛋白质能量ising模型的生物序列设计基准方法的新优化任务。
...
- 负责 | 宋明惠
- 摘要 | 导航对动物行为至关重要，并被认为需要一种对外部环境的内在表现，称为认知地图。这种表示的精确形式通常被认为是空间的度量表示。然而，内部表示是根据其对给定任务性能的贡献来判断的，因此可能在不同类型的导航任务中有所不同。这里我们训练一个循环神经网络来控制一个智能体在一个简单的环境中执行几个导航任务。为了关注内部表示，我们将学习分为一个修改内部连接的任务不可知的预训练阶段和一个控制网络输出的任务特定的Q学习阶段。我们表明，预先训练塑造了网络的吸引子景观，导致连续吸引子、离散吸引子或无序状态。这些结构诱导偏差到q学习阶段，导致跨任务对应的度量和拓扑规律的性能模式。通过将两种类型的网络组合成模块化结构，我们可以得到更好的性能对这两种规则。我们的研究结果表明，在递归网络中，诱导偏差以吸引子景观的形式存在，这种吸引子景观可以通过预先训练形成，并使用动力系统方法进行分析。此外，我们证明了非度量表示对于导航任务是有用的，并且它们与度量表示的结合导致了灵活的多任务学习。
- 创新模式 |  老问题首次引入某种新技术或提出新方法或改进
- 技术总结 | 单智能体， 递归网络
- 关键词 | 导航 递归神经网络
- 源代码 | 暂未发布
- 贡献 | 非度量表示对于导航任务是有用的，并且它们与度量表示的结合提高了多任务学习的灵活性
